...
// In-memory cache: zipCode -> { data, fetchedAt }
const cache = new Map<string, { data: any; fetchedAt: number }>();
const CACHE_TTL_MS = 6 * 60 * 60 * 1000; // 6 hours

async function fetchFredSeries(seriesId: string, limit = 12): Promise<any[]> {
...
function ninetyDayChange(obs: any[]) {
  // obs is sorted desc; find one from ~13 weeks ago
...
serve(async (req) => {
  ...
  try {
    ...
    const [mortgageObs, inventoryObs, domObs, hpiObs, unemployObs] = await Promise.all([
      fetchFredSeries('MORTGAGE30US', 14),       // 30-yr fixed mortgage rate (weekly)
      fetchFredSeries('ACTLISCOUUS', 14),         // Active listing count US (monthly)
      fetchFredSeries('MEDDAYONMARUS', 14),       // Median days on market US (monthly)
      fetchFredSeries('CSUSHPISA', 14),           // Case-Shiller HPI (monthly)
      fetchFredSeries('UNRATE', 14),              // US unemployment rate (monthly)
    ]);
    ...
    // 1. Mortgage rate > 7% and rising → +20
    ...
    // 2. Inventory — use raw count as proxy for months supply (if < national median ~700k = tight)
    ...
    if (dom.current !== null && dom.current > 45) {
      ...
      score += 7;
      factors.push({ label: 'Slowing Velocity', points: 7, reason: 'Days on market is elevated — the market is taking longer to clear, signaling shifting conditions.' });
    }

    // 4. HPI declining over 90 days → +15
    if (hpi90dChange !== null && hpi90dChange < -1) {
      score += 15;
      factors.push({ label: 'Softening Prices', points: 15, reason: `Home prices are down ${Math.abs(hpi90dChange).toFixed(1)}% over 90 days — seller motivation is rising as equity appreciation slows.` });
    ...
    // 5. Unemployment stable or falling → +10
    ...
    // 6. Combined momentum bonus (remaining 20 pts)
    ...
    // ─── Lead Type ───────────────────────────────────────────────────────────
    let leadType: 'seller' | 'transitional' | 'buyer';
    if (score >= 71) leadType = 'seller';
    else if (score >= 41) leadType = 'transitional';
    else leadType = 'buyer';

    // Sort factors by points desc, take top 2
    const topFactors = [...factors].sort((a, b) => b.points - a.points).slice(0, 2);

    // ─── Plain-language interpretations ──────────────────────────────────────
    const mortgageNote = mortgage.current === null
      ? 'Data unavailable'
      : mortgage.trend === 'rising'
      ? `Rates are rising — reducing buyer pool size and increasing seller urgency.`
      : mortgage.trend === 'falling'
      ? `Rates are falling — expanding buyer purchasing power and market activity.`
      : `Rates are holding steady at ${mortgage.current.toFixed(2)}%.`;

    const inventoryNote = inventory.current === null
      ? 'Data unavailable'
      : inventory.trend === 'falling'
      ? `Supply is shrinking — sellers face less competition in a tightening market.`
      : inventory.trend === 'rising'
      ? `Supply is expanding — buyers have more options and sellers face more competition.`
      : `Inventory is stable. Monitor for directional shifts.`;

    const domNote = dom.current === null
      ? 'Data unavailable'
      : dom.current > 45
      ? `Days on market exceeds 45 days — expired listing opportunities are accumulating.`
      : dom.current > 30
      ? `Market velocity is slowing. Listings are taking longer to move.`
      : `Market is moving quickly. Listings are selling in under 30 days on average.`;

    const hpiNote = hpi90dChange === null
      ? 'Data unavailable'
      : hpi90dChange > 2
      ? `Prices are up ${hpi90dChange.toFixed(1)}% over 90 days — strong equity gains are motivating move-up sellers.`
      : hpi90dChange < -1
      ? `Prices are down ${Math.abs(hpi90dChange).toFixed(1)}% over 90 days — seller motivation is rising as equity growth stalls.`
      : `Price momentum is relatively flat. Watch for directional confirmation.`;

    const unemployNote = unemploy.current === null
      ? 'Data unavailable'
      : unemploy.trend === 'falling'
      ? `Unemployment is declining — a strengthening labor market supports household mobility.`
      : unemploy.trend === 'rising'
      ? `Unemployment is rising — economic uncertainty may slow discretionary moves.`
    ...
    const result = {
      ...
      metrics: {
        mortgage: {
          ...
          sublabel: '30-Year Fixed Mortgage Rate',
          current: mortgage.current,
          previous: mortgage.previous,
          trend: mortgage.trend,
          change90d: mortgage90dChange,
          asOfDate: mortgage.asOfDate,
          unit: '%',
          note: mortgageNote,
          flagged: mortgage.current !== null && mortgage.current > 7,
        },
        inventory: {
          seriesId: 'ACTLISCOUUS',
          label: 'Available Supply',
          sublabel: 'Active Housing Listings (US)',
          ...
          unit: 'listings',
          note: inventoryNote,
          flagged: inventoryTight,
        },
        daysOnMarket: {
          seriesId: 'MEDDAYONMARUS',
          label: 'Listing Velocity',
          sublabel: 'Median Days on Market (US)',
          current: dom.current,
          previous: dom.previous,
          trend: dom.trend,
          asOfDate: dom.asOfDate,
          unit: 'days',
          note: domNote,
          flagged: dom.current !== null && dom.current > 45,
        },
        hpi: {
          seriesId: 'CSUSHPISA',
          label: 'Price Momentum',
          sublabel: 'Case-Shiller Home Price Index',
          current: hpi.current,
          previous: hpi.previous,
          trend: hpi.trend,
          change90d: hpi90dChange,
          asOfDate: hpi.asOfDate,
          unit: 'index',
          note: hpiNote,
          flagged: hpi90dChange !== null && hpi90dChange < -1,
        },
        unemployment: {
          seriesId: 'UNRATE',
          label: 'Economic Stability',
          sublabel: 'US Unemployment Rate',
          current: unemploy.current,
          previous: unemploy.previous,
          trend: unemploy.trend,
          asOfDate: unemploy.asOfDate,
          unit: '%',
          note: unemployNote,
