...
const SharedReports = () => {
  ...
  const handleExportPdf = async (session: Session) => {
    ...
    try {
      // Navigate to the shared report in a new tab where the user can use the PDF export button
      // The iframe approach is unreliable because the report must async-fetch data first
      const shareToken = (session as any).share_token || session.id;
      const shareUrl = getShareUrl(shareToken);
      
      // Open the shared report and let the built-in PDF export handle it
      window.open(`${shareUrl}?export=pdf`, '_blank');
      
      toast({
        title: "Report opened",
        description: "Use the PDF button in the report to download.",
      });
    } catch (error) {
      console.error('PDF export failed:', error);
      toast({
        title: "Export failed",
        description: "Could not open report. Try clicking the report to view it first.",
        variant: "destructive",
      });
    } finally {
      setExportingId(null);
