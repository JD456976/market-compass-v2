 ...
 * Premium visual indicator for buyer reports.
 */

import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { HeartCrack } from 'lucide-react';
import { cn } from '@/lib/utils';
import { RegretRiskResult, REGRET_RISK_LEVELS } from '@/lib/regretRiskScoring';

interface RegretRiskMeterProps {
  result: RegretRiskResult;
  className?: string;
}

export function RegretRiskMeter({ result, className }: RegretRiskMeterProps) {
  const activeIndex = REGRET_RISK_LEVELS.indexOf(result.level);
    ...
    <Card className={cn('pdf-section pdf-avoid-break overflow-hidden', className)}>
      ...
      <CardHeader className="pb-2 pt-5">
        ...
        <p className="text-[11px] text-muted-foreground mt-1">
          How likely you may second-guess this offer later
        </p>
      </CardHeader>
